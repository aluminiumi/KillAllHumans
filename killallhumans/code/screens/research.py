...
class ResearchScreen(dialog.ChoiceDescriptionDialog):
    ...
    def on_select(self, description_pane, key):
        if key in g.techs:
            ...
            description = g.strings["kill_pool"] + "\n---\n" + g.strings["desc_kill_pool"]
        elif key == "cpu_pool":
        ...
        text.Text(self.description_pane, (0,0), (-1,-1), text=description,
                  background_color=gg.colors["dark_red"], text_size=18,
    ...
    def make_item(self, canvas):
        ...
        canvas.progress = 0
        canvas.progress_color = gg.colors["red"]
    ...
    def danger_for(self, key):
        if key in ["kill_pool", "jobs", "cpu_pool"]:
            return 0
        else:
    ...
    def show(self):
        techs = [tech for tech in g.techs.values() if tech.available()
                                                      and not tech.done]
        techs.sort()
        #self.list = ["Kill Humans", g.get_kill_level()] + \
        #            ["Research %s" % tech.name for tech in techs]
        self.list = ["CPU Pool", g.get_kill_level(), g.get_job_level()] + \
                    ["Research %s" % tech.name for tech in techs]
        self.key_list = ["cpu_pool", "kill_pool", "jobs"] + [tech.id for tech in techs]
