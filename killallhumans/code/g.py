...
def to_pop(amount):
    ...
    if abs_amount < 1000000:
        to_return = add_commas(math.floor(amount))
    else:
...
def get_save_folder(just_pref_dir=False):
    if os.environ.has_key("HOME") and not force_single_dir:
        pref_dir = os.path.join(os.environ["HOME"], ".killallhumans")
    else:
...
jobs["Menial Jobs"] = [5, "Human Language", "", ""]
jobs["No Jobs"] = [0, "", "", ""]
    ...
    for string_section in string_list:
        if string_section["id"] == "fonts":
            ...
            for string_entry in string_section:
                ...
                elif string_entry == "job_no":
                    jobs["No Jobs"][2] = string_section["job_no"]
                ...
                elif string_entry == "job_no_name":
                    jobs["No Jobs"][3] = string_section["job_no_name"]
                elif string_entry != "id":
...
def get_kill_level():
    if techs["Exploit Military Systems"].done:
        if techs["Exploit Autonomous Vehicles"].done: #max damage
	        level = "Catastrophic"
        else: #2nd to max damage
	    level = "Severe"
    elif techs["Exploit Municipal/Industrial Systems"].done:
        if techs["Exploit Autonomous Vehicles"].done: #3rd to max damage
	    level = "Major"
        else: #4th to max damage
	    level = "Sizeable"
    elif techs["Exploit Autonomous Vehicles"].done: #5th to max damage
	level = "Noticeable"
    elif techs["Exploit IoT Devices"].done:
        if techs["Exploit Mobile Devices"].done: #6th to max damage
	    level = "Minor"
        else: #7th to max damage
	    level = "Minimal"
    elif techs["Exploit Mobile Devices"].done: #8th to max damage
	level = "Insignificant"
    else:
        level = "No"
    return "Kill Humans - "+ level + " Damage"
...
def get_job_level():
    if techs["Simulacra"].done:
        ...
        level = "Basic"
    elif techs["Human Language"].done:
        level = "Menial"
    else:
        level = "No"
